...
const coloresFase = {
    'Acumulacion':   { bg: '#10b981', bgAlpha: 'bg-emerald-500/20', text: 'text-emerald-400', border: 'border-emerald-500/30' },
    'Transmutacion': { bg: '#f59e0b', bgAlpha: 'bg-amber-500/20', text: 'text-amber-400', border: 'border-amber-500/30' },
    'Realizacion':   { bg: '#ef4444', bgAlpha: 'bg-red-500/20', text: 'text-red-400', border: 'border-red-500/30' },
    'Deload':        { bg: '#6b7280', bgAlpha: 'bg-gray-500/20', text: 'text-gray-400', border: 'border-gray-500/30' }
};
...
async function cargarTablaMacro() {
    ...
    if (!datos || datos.length === 0) {
        tbody.innerHTML = `<tr><td colspan="9" class="p-8 text-center text-gray-500">No se encontraron macrociclos.</td></tr>`;
        return;
    }
    ...
    tbody.innerHTML = datos.map(m => `
        <tr class="hover:bg-white/5 transition">
            <td class="p-4">
                <p class="text-white font-medium">${m.nombre || 'Macrociclo #' + m.id_macrociclo}</p>
            </td>
            <td class="p-4 text-xs">${m.temporada_nombre || '-'}</td>
            <td class="p-4 text-xs">${m.grupo_nombre || '-'}</td>
            <td class="p-4 text-xs">
                ${m.evento_objetivo_nombre
                    ? `<span class="text-indigo-300">${m.evento_objetivo_nombre}</span><br><span class="text-[10px] text-gray-500 font-mono">${formatoFecha(m.evento_fecha)}</span>`
                    : '<span class="text-gray-500">Sin asignar</span>'}
            </td>
            <td class="p-4 font-mono text-xs">${formatoFechaRango(m.fecha_inicio, m.fecha_fin)}</td>
            <td class="p-4 text-center">
                <span class="bg-cyan-500/10 text-cyan-400 px-2 py-1 rounded-lg text-xs font-bold">${Math.round(m.total_semanas)} sem</span>
            </td>
            <td class="p-4">
                ${m.fase_actual ? badgeFase(m.fase_actual) : '<span class="text-gray-500 text-xs">Sin generar</span>'}
            </td>
            <td class="p-4">
                <span class="px-2 py-1 rounded-lg text-[10px] font-bold ${coloresEstado[m.estado] || 'bg-gray-500/20 text-gray-400'}">${m.estado}</span>
            </td>
            <td class="p-4 text-right">
                <div class="flex items-center justify-end gap-1">
                    ...
                    <button onclick="abrirModalMacro(${m.id_macrociclo})" class="p-2 text-gray-400 hover:text-white hover:bg-white/10 rounded-lg transition cursor-pointer" title="Editar">
...
async function generarPeriodizacion(id_macrociclo) {
    const { value: formValues } = await Swal.fire({
        ...
        html: `
            <p class="text-sm text-gray-400 mb-4">Configure las proporciones de periodizacion (distribucion inversa desde competencia).</p>
            <div class="grid grid-cols-2 gap-3 text-left">
                <div>
                    <label class="block text-[10px] text-gray-500 uppercase mb-1">Acumulacion (%)</label>
                    <input type="number" id="swp_acum" value="55" min="10" max="90" class="w-full input-dark p-2 rounded-lg text-sm text-center font-mono">
                </div>
                <div>
                    <label class="block text-[10px] text-gray-500 uppercase mb-1">Transmutacion (%)</label>
                    <input type="number" id="swp_trans" value="28" min="5" max="60" class="w-full input-dark p-2 rounded-lg text-sm text-center font-mono">
                </div>
                <div>
                    <label class="block text-[10px] text-gray-500 uppercase mb-1">Realizacion (%)</label>
                    <input type="number" id="swp_real" value="12" min="5" max="40" class="w-full input-dark p-2 rounded-lg text-sm text-center font-mono">
                </div>
                <div>
                    <label class="block text-[10px] text-gray-500 uppercase mb-1">Deload cada (semanas)</label>
                    <input type="number" id="swp_frec" value="4" min="2" max="8" class="w-full input-dark p-2 rounded-lg text-sm text-center font-mono">
                </div>
            </div>
            <div class="mt-4 p-3 bg-black/30 rounded-xl">
                <p class="text-[10px] text-gray-500 uppercase mb-2">Leyenda de Fases</p>
                <div class="flex flex-wrap gap-2">
                    <span class="fase-badge" style="background:rgba(16,185,129,0.2);color:#10b981"><span style="display:inline-block;width:10px;height:10px;border-radius:3px;background:#10b981"></span> Acumulacion</span>
                    <span class="fase-badge" style="background:rgba(245,158,11,0.2);color:#f59e0b"><span style="display:inline-block;width:10px;height:10px;border-radius:3px;background:#f59e0b"></span> Transmutacion</span>
                    <span class="fase-badge" style="background:rgba(239,68,68,0.2);color:#ef4444"><span style="display:inline-block;width:10px;height:10px;border-radius:3px;background:#ef4444"></span> Realizacion</span>
                    <span class="fase-badge" style="background:rgba(107,114,128,0.2);color:#6b7280"><span style="display:inline-block;width:10px;height:10px;border-radius:3px;background:#6b7280"></span> Deload</span>
                </div>
            </div>
        `,
    ...
    if (resultado) {
        if (resultado.status === 'success') {
            ...
            if (data.advertencias && data.advertencias.length > 0) {
                mensajeAdvertencias = '<br><br><span class="text-amber-400 text-xs"><i class="fas fa-exclamation-triangle mr-1"></i> Advertencias de desviacion:</span><br>' +
                    data.advertencias.map(a => `<span class="text-amber-300 text-xs block ml-4">- ${a}</span>`).join('');
            }
            ...
            await Swal.fire({
                ...
                title: 'Plan Generado',
                html: `
                    <p class="text-sm text-gray-300">${resultado.message}</p>
                    <div class="mt-3 grid grid-cols-4 gap-2 text-center">
                        <div class="p-2 bg-black/30 rounded-lg">
                            <p class="text-emerald-400 font-bold">${data.porcentajes.acumulacion_generado}%</p>
                            <p class="text-[9px] text-gray-500 uppercase">Acumulacion</p>
                        </div>
                        <div class="p-2 bg-black/30 rounded-lg">
                            <p class="text-amber-400 font-bold">${data.porcentajes.transmutacion_generado}%</p>
                            <p class="text-[9px] text-gray-500 uppercase">Transmutacion</p>
                        </div>
                        <div class="p-2 bg-black/30 rounded-lg">
                            <p class="text-red-400 font-bold">${data.porcentajes.realizacion_generado}%</p>
                            <p class="text-[9px] text-gray-500 uppercase">Realizacion</p>
                        </div>
                        <div class="p-2 bg-black/30 rounded-lg">
                            <p class="text-gray-400 font-bold">${data.porcentajes.deload_generado}%</p>
                            <p class="text-[9px] text-gray-500 uppercase">Deload</p>
                        </div>
                    </div>
                    <p class="text-xs text-gray-500 mt-2">Total: ${data.total_semanas} semanas</p>
                    ${mensajeAdvertencias}
                `
            });
            cargarTablaMacro();
...
async function verDetalle(id_macrociclo) {
    ...
    let html = `
        <div class="mb-6">
            <h2 class="text-xl font-bold text-white mb-1">${detalle.nombre || 'Macrociclo #' + detalle.id_macrociclo}</h2>
            <div class="flex flex-wrap gap-2 mt-2">
                <span class="px-2 py-1 rounded-lg text-[10px] font-bold ${coloresEstado[detalle.estado] || 'bg-gray-500/20 text-gray-400'}">${detalle.estado}</span>
                <span class="px-2 py-1 rounded-lg text-[10px] font-bold bg-cyan-500/20 text-cyan-400">${detalle.temporada_nombre}</span>
                <span class="px-2 py-1 rounded-lg text-[10px] font-bold bg-purple-500/20 text-purple-400">${detalle.grupo_nombre}</span>
                ${detalle.evento_objetivo_nombre ? `<span class="px-2 py-1 rounded-lg text-[10px] font-bold bg-indigo-500/20 text-indigo-400"><i class="fas fa-trophy mr-1"></i>${detalle.evento_objetivo_nombre}</span>` : ''}
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 p-4 bg-black/20 rounded-xl">
            ...
            <div><p class="text-[10px] text-gray-500 uppercase">Duracion</p><p class="text-white text-sm">${totalSemanas} semanas</p></div>
            <div><p class="text-[10px] text-gray-500 uppercase">Evento Objetivo</p><p class="text-white text-sm">${detalle.evento_objetivo_nombre || 'Sin asignar'}</p></div>
        </div>
    `;
    ...
    if (detalle.fases && detalle.fases.length > 0) {
        ...
        html += `
            <div class="mb-8">
                <h4 class="text-sm font-bold text-white mb-4"><i class="fas fa-chart-gantt mr-2 text-indigo-400"></i>Timeline de Periodizacion</h4>
                <div class="px-1 pb-6">
                    ${timelineHtml}
                </div>
            </div>
        ...
        html += `
            <div class="mb-6">
                <h4 class="text-sm font-bold text-white mb-3"><i class="fas fa-layer-group mr-2 text-indigo-400"></i>Detalle de Fases</h4>
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs">
                        ...
                        <tbody class="divide-y divide-[#252345] text-gray-300">
                            ${detalle.fases.map(f => `
                                <tr class="${f.fecha_inicio <= hoy && f.fecha_fin >= hoy ? 'bg-white/5' : ''}">
    ...
    if (detalle.mesociclos && detalle.mesociclos.length > 0) {
        html += `
            <div class="mb-6">
                <h4 class="text-sm font-bold text-white mb-3"><i class="fas fa-cubes mr-2 text-amber-400"></i>Mesociclos</h4>
                <div class="space-y-3">
                    ${detalle.mesociclos.map(m => `
                        <div class="p-4 bg-black/20 rounded-xl border-l-4" style="border-color: ${m.fase_color || '#6366f1'}">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    <span class="text-white font-medium text-sm">${m.nombre || 'Mesociclo'}</span>
                            ...
                            ${m.objetivo ? `<p class="text-xs text-gray-400 mb-2">${m.objetivo}</p>` : ''}
                            ${m.volumen_objetivo_m ? `<p class="text-xs text-gray-500"><i class="fas fa-ruler-horizontal mr-1"></i>Volumen objetivo: ${Number(m.volumen_objetivo_m).toLocaleString()}m</p>` : ''}
                            ${m.microciclos && m.microciclos.length > 0 ? `
                                <div class="mt-3 pt-3 border-t border-[#252345]">
                                    <p class="text-[10px] text-gray-500 uppercase mb-2">Microciclos (${m.microciclos.length})</p>
                                    <div class="flex flex-wrap gap-2">
                                        ${m.microciclos.map(mi => `
                                            <div class="p-2 bg-[#0f0d23] rounded-lg text-[10px]">
                                                <span class="text-gray-400">S${mi.numero_semana}</span>
                                                <span class="text-gray-500 ml-1">${formatoFecha(mi.fecha_inicio)}</span>
                                                ${mi.volumen_planificado_m ? `<span class="text-indigo-300 ml-1">${Number(mi.volumen_planificado_m).toLocaleString()}m</span>` : ''}
                                            </div>
                                        `).join('')}
                                    </div>
                                </div>
    ...
    if (!detalle.fases || detalle.fases.length === 0) {
        html += `
            <div class="text-center py-8">
                ...
                <p class="text-gray-400 mb-4">Este macrociclo aun no tiene plan de periodizacion generado.</p>
        ...
        return `
            <div class="timeline-bar ${esActiva ? 'fase-activa' : ''}"
                ...
                <div class="tooltip-timeline">
                    <p class="font-bold text-white">${f.nombre_fase}</p>
                    <p class="text-gray-400 mt-1">Sem ${f.semana_inicio} - ${f.semana_fin} (${numSemanas} sem)</p>
                    <p class="text-gray-400">Volumen: ${f.porcentaje_volumen}% | Intensidad: ${f.rango_intensidad}</p>
                    <p class="text-gray-500 text-[10px]">${formatoFecha(f.fecha_inicio)} al ${formatoFecha(f.fecha_fin)}</p>
                    ${esActiva ? '<p class="text-emerald-400 font-bold mt-1"><i class="fas fa-circle text-[8px] mr-1"></i>Fase Activa</p>' : ''}
                </div>
            </div>
        `;
    }).join('');
