...
<head>
    ...
    <title>Observaciones Técnicas | SGRD</title>
    ...
    <style>
        ...
        .dark .c1 { background: #dc262620; color: #f87171; border-color: #dc262640; }
        .c2 { background: #f59e0b20; color: #fbbf24; border-color: #f59e0b40; }
        .dark .c2 { background: #f59e0b20; color: #fbbf24; border-color: #f59e0b40; }
        .c3 { background: #eab30820; color: #facc15; border-color: #eab30840; }
        .dark .c3 { background: #eab30820; color: #facc15; border-color: #eab30840; }
        .c4 { background: #22c55e20; color: #4ade80; border-color: #22c55e40; }
        .dark .c4 { background: #22c55e20; color: #4ade80; border-color: #22c55e40; }
        .c5 { background: #16a34a20; color: #22c55e; border-color: #16a34a40; }
        .dark .c5 { background: #16a34a20; color: #22c55e; border-color: #16a34a40; }
...
?>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
            ...
            <?php
                ...
                $tituloPaginaResponsive = "Obs. Técnicas";
            ...
            <main class="flex-grow p-4 sm:p-6 lg:p-8 max-w-[1600px] w-full mx-auto space-y-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-[#161430] p-6 rounded-2xl border border-gray-200 dark:border-[#252345] transition-colors duration-300">
                    <div>
                        <h2 class="text-xl sm:text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight flex items-center gap-2">
                            <i class="fas fa-clipboard-check text-indigo-500"></i> Evaluaciones Técnicas
                        </h2>
                        <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">Evaluaciones cualitativas de la técnica del nadador por sesión.</p>
                    </div>
                    ...
                    <button onclick="abrirModalObservacion()" class="w-full sm:w-auto px-5 py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-xs tracking-wider uppercase shadow-lg shadow-indigo-500/20 transition-all duration-300 transform hover:-translate-y-0.5 flex items-center justify-center gap-2 cursor-pointer">
                        <i class="fas fa-plus-circle text-sm"></i> Registrar Observación
                    </button>
                    <?php endif; ?>
                </div>
                ...
                <div class="bg-white dark:bg-[#161430] border border-gray-200 dark:border-[#252345] rounded-2xl p-5 transition-colors duration-300">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="fas fa-crosshairs text-cyan-600 dark:text-cyan-400/70 text-xs"></i>
                            </div>
                            <select id="filtroAspecto" onchange="cargarTabla()" class="w-full input-adapt pl-9 pr-8 py-2.5 rounded-xl text-xs appearance-none cursor-pointer">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="busquedaGeneral" placeholder="Buscar en observaciones..." class="w-full input-adapt pl-9 pr-4 py-2.5 rounded-xl text-xs" oninput="filtrarTabla()">
                        </div>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="fas fa-chart-bar text-emerald-600 dark:text-emerald-400/70 text-xs"></i>
                            </div>
                            <select id="filtroAtletaResumen" class="w-full input-adapt pl-9 pr-8 py-2.5 rounded-xl text-xs appearance-none cursor-pointer">
                ...
                <div class="bg-white dark:bg-[#161430] border border-gray-200 dark:border-[#252345] rounded-2xl overflow-hidden shadow-2xl transition-colors duration-300">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody id="tbodyObservaciones" class="divide-y divide-gray-200 dark:divide-[#252345] text-sm text-gray-800 dark:text-gray-300">
                            </tbody>
                        </table>
                    </div>
                </div>
    ...
    <div id="modalObservacion" class="fixed inset-0 z-50 hidden bg-black/20 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="relative bg-white dark:bg-[#161430] border border-gray-200 dark:border-white/5 w-full max-w-2xl rounded-2xl shadow-2xl transform scale-95 opacity-0 transition-all duration-300 max-h-[92vh] overflow-y-auto p-6 md:p-8 transition-colors duration-300">
            ...
            <div class="flex justify-between items-center mb-6 border-b border-gray-200 dark:border-gray-800 pb-4">
                <h3 id="modalTitulo" class="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    <i class="fas fa-clipboard-check text-indigo-400"></i> Registrar Observación Técnica
                </h3>
                <button onclick="cerrarModalObservacion()" class="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition cursor-pointer">
            ...
            <form id="formObservacion" autocomplete="off">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-xs text-gray-600 dark:text-gray-400 uppercase font-bold mb-2">Aspecto Técnico *</label>
                        <select id="id_aspecto_tecnico" name="id_aspecto_tecnico" data-validar="requerido" data-nombre="Aspecto Técnico" class="w-full input-adapt p-3 rounded-xl text-sm cursor-pointer">
                            <option value="">Seleccione un aspecto...</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-xs text-gray-600 dark:text-gray-400 uppercase font-bold mb-2">Sesión (Opcional)</label>
                        <select id="id_sesion" name="id_sesion" class="w-full input-adapt p-3 rounded-xl text-sm cursor-pointer">
                            <option value="">Sin sesión asociada</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-xs text-gray-600 dark:text-gray-400 uppercase font-bold mb-2">Calificación *</label>
                        ...
                        <p id="textoCalificacion" class="text-[10px] text-gray-500 dark:text-gray-400 mt-1">1=Necesita trabajo | 2=Regular | 3=Bueno | 4=Muy bueno | 5=Excelente</p>
                    </div>
                </div>

                <div class="mt-4">
                    <label class="block text-xs text-gray-600 dark:text-gray-400 uppercase font-bold mb-2">Observación Textual</label>
                    <textarea id="observacion_texto" name="observacion_texto" data-validar="texto" data-max="500" data-nombre="Observación" rows="3" maxlength="500" placeholder="Notas detalladas sobre la técnica observada..." class="w-full input-adapt p-3 rounded-xl text-sm"></textarea>
                </div>

                <div class="flex gap-3 mt-6">
                    <button type="button" onclick="cerrarModalObservacion()" class="flex-1 bg-gray-200 hover:bg-gray-300 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 py-3.5 rounded-xl font-bold transition cursor-pointer uppercase text-xs tracking-wider">CANCELAR</button>
                    <button type="submit" id="btnGuardar" class="flex-[2] bg-indigo-600 hover:bg-indigo-500 text-white py-3.5 rounded-xl font-bold shadow-lg shadow-indigo-500/20 cursor-pointer uppercase text-xs tracking-wider">
                        GUARDAR OBSERVACIÓN <i class="fas fa-save ml-2"></i>
                    </button>
                </div>
            </form>
        </div>
    ...
    <!-- ===== MODAL RESUMEN ===== -->
    <div id="modalResumen" class="fixed inset-0 z-50 hidden bg-black/20 dark:bg-[#060512]/90 backdrop-blur-xl flex items-center justify-center p-4">
        <div class="relative bg-white dark:bg-[#111026] border border-gray-200 dark:border-white/10 w-full max-w-3xl rounded-[2rem] overflow-hidden shadow-[0_0_50px_rgba(79,70,229,0.15)] max-h-[92vh] overflow-y-auto transition-colors duration-300">
            <button type="button" onclick="cerrarModalResumen()" class="absolute top-6 right-6 text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white hover:rotate-90 transition-all duration-300 z-[100] cursor-pointer p-2">
                <i class="fas fa-times text-2xl"></i>
            </button>
