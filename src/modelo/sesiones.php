<?php

namespace GrupoProyecto\SisBiomec\modelo;

use PDO;
use PDOException;

class Sesiones extends Conexion {
    use ValidacionesTrait;

    private const CAMPOS_SESION = ['id_sesion', 'id_microciclo', 'id_grupo', 'id_entrenador', 'fecha', 'tipo_sesion',
    'duracion_minutos', 'calentamiento', 'vuelta_calma', 'volumen_planificado', 'volumen_ejecutado', 'observaciones', 'estado',
    ];

    private const CAMPOS_SERIE = ['id_serie', 'id_sesion', 'orden_ejecucion', 'bloque', 'id_drill', 'ejercicio_descripcion', 'repeticiones',
        'distancia_m', 'descanso_seg', 'zona_intensidad', 'ritmo_objetivo',
    ];

    private const TIPOS_SESION = ['Tecnica', 'Resistencia', 'Velocidad', 'Recuperacion',
        'Fuerza', 'Flexibilidad', 'Competencia',
    ];

    private const BLOQUES_VALIDOS = ['Calentamiento', 'Principal', 'VuletaCalma'];
    private const ZONAS_VALIDAS = ['Z1', 'Z2', 'Z3', 'Z4', 'Z5'];

    private array $datos = [];
    private array $series = [];

    public function __construct() {
        parent::__construct('sis_natacion');
    }

    public function setDatos(array $datos): void {
        $this->datos = array_intersect_key($datos, array_flip(self::CAMPOS_SESION));
    }

    public function setSeries(array $series): void {
        $this->series = [];
        foreach ($series as $serie) {
            if (is_array($serie)) {
                $this->series[] = array_intersect_key($serie, array_flip(self::CAMPOS_SERIE));
            }
        }
    }

    public function getCampo(string $clave) {
        return $this->datos[$clave] ?? null;
    }

    public function validarDatos($excluirId = null): array {
        $this->resetearErrores();

        $id_entrenador = $this->datos['id_entrenador'] ?? '';
        $this->requerido($id_entrenador, 'id_entrenador');
        if (!empty($id_entrenador)) {
            $this->soloNumeros($id_entrenador, 'id_entrenador');
        }

        $id_grupo = $this->datos['id_grupo'] ?? '';
        $this->requerido($id_grupo, 'id_grupo');
        if (!empty($id_grupo)) {
            $this->soloNumeros($id_grupo, 'id_grupo');
            if ((int)$id_grupo <= 0) {
                $this->agregarError('id_grupo', 'El ID del grupo debe ser un número válido mayor a 0.');
            }
        }

        if (!empty($this->datos['id_microciclo'])) {
            $this->soloNumeros($this->datos['id_microciclo'], 'id_microciclo');
            if ((int)$this->datos['id_microciclo'] <= 0) {
                $this->agregarError('id_microciclo', 'El ID del microciclo debe ser un número válido mayor a 0.');
            }
        }

        $fecha = $this->datos['fecha'] ?? '';
        $this->requerido($fecha, 'fecha');
        if (!empty($fecha)) {
            $this->fechaValida($fecha, 'fecha');
            if ($excluirId === null && $fecha < date('Y-m-d')) {
                $this->agregarError('fecha', 'No se permite planificar sesiones para fechas pasadas.');
            }
        }

        $tipo_sesion = $this->datos['tipo_sesion'] ?? '';
        $this->requerido($tipo_sesion, 'tipo_sesion');
        if (!empty($tipo_sesion)) {
            $this->enEnum($tipo_sesion, 'tipo_sesion', self::TIPOS_SESION);
        }

        $duracion = $this->datos['duracion_minutos'] ?? '';
        $this->requerido($duracion, 'duracion_minutos');
        if (!empty($duracion)) {
            $this->soloNumeros($duracion, 'duracion_minutos');
            if ((int)$duracion < 15) {
                $this->agregarError('duracion_minutos', 'La duración mínima de la sesión debe ser de 15 minutos.');
            }
            if ((int)$duracion > 480) {
                $this->agregarError('duracion_minutos', 'La duración máxima de la sesión es de 480 minutos (8 horas).');
            }
        }

        $calentamiento = $this->datos['calentamiento'] ?? '';
        $this->longitud($calentamiento, 'calentamiento', 0, 100);
        if (!empty($calentamiento) && !preg_match('/^[a-zA-ZáéíóúÁÉÍÓÚñÑ0-9\s.,;:()\-_\n]+$/', $calentamiento)) {
            $this->agregarError('calentamiento', 'El calentamiento contiene caracteres no permitidos.');
        }

        $vuelta_calma = $this->datos['vuelta_calma'] ?? '';
        $this->longitud($vuelta_calma, 'vuelta_calma', 0, 100);
        if (!empty($vuelta_calma) && !preg_match('/^[a-zA-ZáéíóúÁÉÍÓÚñÑ0-9\s.,;:()\-_\n]+$/', $vuelta_calma)) {
            $this->agregarError('vuelta_calma', 'La vuelta a la calma contiene caracteres no permitidos.');
        }

        $observaciones = $this->datos['observaciones'] ?? '';
        $this->longitud($observaciones, 'observaciones', 0, 500);
        if (!empty($observaciones) && !preg_match('/^[a-zA-ZáéíóúÁÉÍÓÚñÑ0-9\s.,;:()\-_\n]+$/', $observaciones)) {
            $this->agregarError('observaciones', 'Las observaciones contienen caracteres no permitidos.');
        }

        return $this->obtenerErrores();
    }

    public function validarSerie(array $serie): array {
        $this->resetearErrores();

        $bloque = $serie['bloque'] ?? '';
        $this->requerido($bloque, 'bloque');
        if (!empty($bloque)) {
            $this->enEnum($bloque, 'bloque', self::BLOQUES_VALIDOS);
        }

        $id_drill = $serie['id_drill'] ?? '';
        $ejercicio_descripcion = $serie['ejercicio_descripcion'] ?? '';

        if (empty($id_drill) && empty($ejercicio_descripcion)) {
            $this->agregarError('ejercicio_descripcion', 'Debe seleccionar un drill o describir el ejercicio.');
        }

        if (!empty($id_drill)) {
            $this->soloNumeros($id_drill, 'id_drill');
        }

        if (!empty($ejercicio_descripcion)) {
            $this->longitud($ejercicio_descripcion, 'ejercicio_descripcion', 3, 500);
            if (!preg_match('/^[a-zA-ZáéíóúÁÉÍÓÚñÑ0-9\s.,;:()\-_\n]+$/', $ejercicio_descripcion)) {
                $this->agregarError('ejercicio_descripcion', 'La descripción contiene caracteres no permitidos.');
            }
        }

        $repeticiones = $serie['repeticiones'] ?? '';
        $this->requerido($repeticiones, 'repeticiones');
        if (!empty($repeticiones)) {
            $this->soloNumeros($repeticiones, 'repeticiones');
            if ((int)$repeticiones < 1) {
                $this->agregarError('repeticiones', 'Las repeticiones mínimas deben ser 1.');
            }
            if ((int)$repeticiones > 100) {
                $this->agregarError('repeticiones', 'Las repeticiones máximas son 100.');
            }
        }

        $distancia = $serie['distancia_m'] ?? '';
        $this->requerido($distancia, 'distancia_m');
        if (!empty($distancia)) {
            $this->soloNumeros($distancia, 'distancia_m');
            if ((int)$distancia < 25) {
                $this->agregarError('distancia_m', 'La distancia mínima por repetición es de 25 metros.');
            }
            if ((int)$distancia > 10000) {
                $this->agregarError('distancia_m', 'La distancia máxima por repetición es de 10000 metros.');
            }
        }

        $descanso = $serie['descanso_seg'] ?? '';
        $this->requerido($descanso, 'descanso_seg');
        if (!empty($descanso)) {
            $this->soloNumeros($descanso, 'descanso_seg');
            if ((int)$descanso > 600) {
                $this->agregarError('descanso_seg', 'El descanso máximo es de 600 segundos (10 minutos).');
            }
        }

        $zona = $serie['zona_intensidad'] ?? '';
        $this->requerido($zona, 'zona_intensidad');
        if (!empty($zona)) {
            $this->enEnum($zona, 'zona_intensidad', self::ZONAS_VALIDAS);
        }

        if (!empty($serie['ritmo_objetivo']) && strlen($serie['ritmo_objetivo']) > 20) {
            $this->agregarError('ritmo_objetivo', 'El ritmo objetivo no puede exceder los 20 caracteres.');
        }

        return $this->obtenerErrores();
    }

    public function validarCierreSesion(int $volumenPlanificado): array {
        $this->resetearErrores();

        $volumen_ejecutado = $this->datos['volumen_ejecutado'] ?? '';
        $this->requerido($volumen_ejecutado, 'volumen_ejecutado');
        if (!empty($volumen_ejecutado)) {
            $this->soloNumeros($volumen_ejecutado, 'volumen_ejecutado');
            if ((int)$volumen_ejecutado < 0) {
                $this->agregarError('volumen_ejecutado', 'El volumen ejecutado no puede ser negativo.');
            }
            if ((int)$volumen_ejecutado > $volumenPlanificado) {
                $this->agregarError('volumen_ejecutado', 'El volumen ejecutado no puede superar al volumen planificado (' . $volumenPlanificado . 'm).');
            }
        }

        if (!empty($this->datos['observaciones'])) {
            $observaciones = $this->datos['observaciones'];
            $this->longitud($observaciones, 'observaciones', 0, 5000);
            if (!preg_match('/^[a-zA-ZáéíóúÁÉÍÓÚñÑ0-9\s.,;:()\-_\n]+$/', $observaciones)) {
                $this->agregarError('observaciones', 'Las observaciones contienen caracteres no permitidos.');
            }
        }

        return $this->obtenerErrores();
    }

    public function registrarSesion(): bool {
        $errores = $this->validarDatos();
        if (!empty($errores)) {
            return false;
        }
        foreach ($this->series as $serie) {
            $errSerie = $this->validarSerie($serie);
            if (!empty($errSerie)) {
                return false;
            }
        }
        return $this->registrarSesionP();
    }

    public function editarSesion(): bool {
        $id_sesion = (int)($this->datos['id_sesion'] ?? 0);
        if ($id_sesion <= 0) {
            return false;
        }
        $errores = $this->validarDatos($id_sesion);
        if (!empty($errores)) {
            return false;
        }
        foreach ($this->series as $serie) {
            $errSerie = $this->validarSerie($serie);
            if (!empty($errSerie)) {
                return false;
            }
        }
        return $this->editarSesionP();
    }

    public function InicializarSesion(): bool {
        $id = (int)($this->datos['id_sesion'] ?? 0);
        if ($id <= 0) {
            $this->agregarError('id_sesion', 'No se proporcionó un identificador válido para actualizar el registro.');
            return false;
        }
        return $this->iniciarSesionP();
    }

    public function completarSesion(): bool {
        $id_sesion = (int)($this->datos['id_sesion'] ?? 0);
        if ($id_sesion <= 0) {
            return false;
        }

        $detalle = $this->obtenerDetalleSesion($id_sesion);
        if (!$detalle) {
            return false;
        }

        $volumenPlanificado = (int)($detalle['volumen_planificado'] ?? 0);
        $errores = $this->validarCierreSesion($volumenPlanificado);
        if (!empty($errores)) {
            return false;
        }

        return $this->completarSesionP();
    }

    public function cancelarSesion(): bool {
        $id_sesion = (int)($this->datos['id_sesion'] ?? 0);
        if ($id_sesion <= 0) {
            return false;
        }
        return $this->cancelarSesionP($id_sesion);
    }

    public function obtenerDetalleSesion(int $id_sesion): ?array {
        if ($id_sesion <= 0) {
            return null;
        }
        $conex = $this->getConex1();
        try {
            $sqlBase = "SELECT s.*, g.nombre as grupo_nombre
                        FROM sesiones s
                        INNER JOIN grupos_entrenamiento g ON s.id_grupo = g.id_grupo
                        WHERE s.id_sesion = :id";
            $stmtBase = $conex->prepare($sqlBase);
            $stmtBase->execute([':id' => $id_sesion]);
            $sesion = $stmtBase->fetch(PDO::FETCH_ASSOC);

            if (!$sesion) return null;

            $sqlSeries = "SELECT sd.*, d.nombre as drill_nombre, d.estilo as drill_estilo
                          FROM series_sesion sd
                          LEFT JOIN drills d ON sd.id_drill = d.id_drill
                          WHERE sd.id_sesion = :id
                          ORDER BY sd.orden_ejecucion ASC";
            $stmtSeries = $conex->prepare($sqlSeries);
            $stmtSeries->execute([':id' => $id_sesion]);
            $sesion['series'] = $stmtSeries->fetchAll(PDO::FETCH_ASSOC);

            return $sesion;
        } catch (PDOException $e) {
            error_log("Error en obtenerDetalleSesion: " . $e->getMessage());
            return null;
        }
    }

    public function listarSesiones(?int $id_grupo = null, ?string $estado = null): array {
        $conex = $this->getConex1();
        try {
            $sql = "SELECT s.*, g.nombre as grupo_nombre
                    FROM sesiones s
                    INNER JOIN grupos_entrenamiento g ON s.id_grupo = g.id_grupo
                    WHERE 1=1";
            $params = [];

            if ($id_grupo) {
                $sql .= " AND s.id_grupo = :id_grupo";
                $params[':id_grupo'] = $id_grupo;
            }
            if ($estado) {
                $sql .= " AND s.estado = :estado";
                $params[':estado'] = $estado;
            }
            $sql .= " ORDER BY s.fecha DESC, s.id_sesion DESC";

            $stmt = $conex->prepare($sql);
            $stmt->execute($params);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            error_log("Error listando sesiones: " . $e->getMessage());
            return [];
        }
    }

    public function obtenerSeriesSesion(int $id_sesion): array {
        if ($id_sesion <= 0) {
            return [];
        }
        $conex = $this->getConex1();
        try {
            $sql = "SELECT sd.*, d.nombre as drill_nombre, d.estilo as drill_estilo
                    FROM series_sesion sd
                    LEFT JOIN drills d ON sd.id_drill = d.id_drill
                    WHERE sd.id_sesion = :id_sesion
                    ORDER BY sd.orden_ejecucion ASC";
            $stmt = $conex->prepare($sql);
            $stmt->execute([':id_sesion' => $id_sesion]);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            error_log("Error en obtenerSeriesSesion: " . $e->getMessage());
            return [];
        }
    }

    public function listarMicrociclos(): array {
        $conex = $this->getConex1();
        try {
            $sql = "SELECT id_microciclo, nombre FROM microciclos ORDER BY nombre";
            $stmt = $conex->query($sql);
            return $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            error_log("Error listando microciclos: " . $e->getMessage());
            return [];
        }
    }

    public function listarSesionesSelectMarca(): array {
        $conex = $this->getConex1();
        try {
            $sql = "SELECT s.id_sesion, s.fecha, s.tipo_sesion, g.nombre AS grupo_nombre
                    FROM sesiones s
                    INNER JOIN grupos_entrenamiento g ON s.id_grupo = g.id_grupo
                    WHERE s.estado IN ('Completada', 'Parcial')
                    ORDER BY s.fecha DESC LIMIT 30";
            return $conex->query($sql)->fetchAll(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            error_log("Error listando sesiones para marcas: " . $e->getMessage());
            return [];
        }
    }

    public function validarDatosSesion(): array {
        return $this->validarDatos();
    }

    private function iniciarSesionP(): bool {
        $conex = $this->getConex1();
        try {
            $id_sesion = (int)$this->datos['id_sesion'];

            $sql = "UPDATE sesiones SET estado = 'Parcial', fecha_modificacion = NOW()
                    WHERE id_sesion = :id_sesion AND estado = 'Planificada'";
            $stmt = $conex->prepare($sql);
            $resultado = $stmt->execute([':id_sesion' => $id_sesion]);

            if ($resultado) {
                $sqlInfo = "SELECT id_grupo FROM sesiones WHERE id_sesion = :id_sesion";
                $stmtInfo = $conex->prepare($sqlInfo);
                $stmtInfo->execute([':id_sesion' => $id_sesion]);
                $sesion = $stmtInfo->fetch(PDO::FETCH_ASSOC);

                if ($sesion) {
                    $this->notificarEventoSesion('iniciar_sesion', [
                        'id_sesion' => $id_sesion,
                        'id_grupo'  => $sesion['id_grupo'],
                    ]);
                }
            }

            return $resultado;
        } catch (PDOException $e) {
            error_log("Error en iniciarSesionP: " . $e->getMessage());
            return false;
        }
    }

    private function completarSesionP(): bool {
        $conex = $this->getConex1();
        try {
            $id_sesion = (int)$this->datos['id_sesion'];

            $sql = "UPDATE sesiones SET
                        volumen_ejecutado = :volumen_ejecutado,
                        estado = :estado,
                        observaciones = :observaciones,
                        fecha_modificacion = NOW()
                    WHERE id_sesion = :id_sesion";
            $stmt = $conex->prepare($sql);
            $resultado = $stmt->execute([
                ':volumen_ejecutado' => (int)($this->datos['volumen_ejecutado'] ?? 0),
                ':estado'            => $this->datos['estado'] ?? 'Completada',
                ':observaciones'     => $this->datos['observaciones'] ?? null,
                ':id_sesion'         => $id_sesion,
            ]);

            if ($resultado) {
                $sqlInfo = "SELECT id_grupo FROM sesiones WHERE id_sesion = :id_sesion";
                $stmtInfo = $conex->prepare($sqlInfo);
                $stmtInfo->execute([':id_sesion' => $id_sesion]);
                $sesion = $stmtInfo->fetch(PDO::FETCH_ASSOC);

                if ($sesion) {
                    $this->notificarEventoSesion('completar_sesion', [
                        'id_sesion'         => $id_sesion,
                        'id_grupo'          => $sesion['id_grupo'],
                        'volumen_ejecutado' => (int)($this->datos['volumen_ejecutado'] ?? 0),
                    ]);
                }
            }

            return $resultado;
        } catch (PDOException $e) {
            error_log("Error en completarSesionP: " . $e->getMessage());
            return false;
        }
    }

    private function cancelarSesionP(int $id_sesion): bool {
        $conex = $this->getConex1();
        try {
            $sqlInfo = "SELECT id_grupo, fecha FROM sesiones WHERE id_sesion = :id_sesion";
            $stmtInfo = $conex->prepare($sqlInfo);
            $stmtInfo->execute([':id_sesion' => $id_sesion]);
            $sesion = $stmtInfo->fetch(PDO::FETCH_ASSOC);

            $sql = "UPDATE sesiones SET estado = 'Cancelada', fecha_modificacion = NOW()
                    WHERE id_sesion = :id_sesion";
            $stmt = $conex->prepare($sql);
            $resultado = $stmt->execute([':id_sesion' => $id_sesion]);

            if ($resultado && $sesion) {
                $this->notificarEventoSesion('cancelar_sesion', [
                    'id_sesion' => $id_sesion,
                    'id_grupo'  => $sesion['id_grupo'],
                    'fecha'     => $sesion['fecha'],
                ]);
            }

            return $resultado;
        } catch (PDOException $e) {
            error_log("Error en cancelarSesionP: " . $e->getMessage());
            return false;
        }
    }

    private function registrarSesionP(): bool {
        $conex = $this->getConex1();
        try {
            $conex->beginTransaction();

            $volumenes = $this->calcularVolumenes($this->series);
            $volumen_planificado = $volumenes['total'];

            $sql = "INSERT INTO sesiones (
                        id_microciclo, id_grupo, fecha, tipo_sesion,
                        calentamiento, vuelta_calma,
                        volumen_planificado, observaciones, estado, id_entrenador, duracion_minutos
                    ) VALUES (
                        :id_microciclo, :id_grupo, :fecha, :tipo_sesion,
                        :calentamiento, :vuelta_calma,
                        :volumen_planificado, :observaciones, 'Planificada', :id_entrenador, :duracion_minutos
                    )";
            $stmt = $conex->prepare($sql);
            $stmt->execute([
                ':id_microciclo'       => !empty($this->datos['id_microciclo']) ? (int)$this->datos['id_microciclo'] : null,
                ':id_grupo'            => (int)$this->datos['id_grupo'],
                ':fecha'               => $this->datos['fecha'],
                ':tipo_sesion'         => $this->datos['tipo_sesion'],
                ':calentamiento'       => $this->datos['calentamiento'] ?? null,
                ':vuelta_calma'        => $this->datos['vuelta_calma'] ?? null,
                ':volumen_planificado' => $volumen_planificado,
                ':observaciones'       => $this->datos['observaciones'] ?? null,
                ':id_entrenador'       => (int)$this->datos['id_entrenador'],
                ':duracion_minutos'    => $this->datos['duracion_minutos'] ?? null,
            ]);

            $id_sesion = (int)$conex->lastInsertId();
            $this->insertarSeries($conex, $id_sesion);

            $conex->commit();

            $this->notificarEventoSesion('crear_sesion', [
                'id_sesion'   => $id_sesion,
                'id_grupo'    => (int)$this->datos['id_grupo'],
                'fecha'       => $this->datos['fecha'],
                'tipo_sesion' => $this->datos['tipo_sesion'],
            ]);

            return true;
        } catch (PDOException $e) {
            if ($conex->inTransaction()) {
                $conex->rollBack();
            }
            error_log("Error en registrarSesionP: " . $e->getMessage());
            return false;
        }
    }

    private function editarSesionP(): bool {
        $conex = $this->getConex1();
        try {
            $conex->beginTransaction();

            $id_sesion = (int)$this->datos['id_sesion'];
            $volumenes = $this->calcularVolumenes($this->series);
            $volumen_planificado = $volumenes['total'];

            $sql = "UPDATE sesiones SET
                        id_microciclo = :id_microciclo,
                        id_grupo = :id_grupo,
                        fecha = :fecha,
                        tipo_sesion = :tipo_sesion,
                        calentamiento = :calentamiento,
                        vuelta_calma = :vuelta_calma,
                        volumen_planificado = :volumen_planificado,
                        observaciones = :observaciones,
                        id_entrenador = :id_entrenador,
                        duracion_minutos = :duracion_minutos,
                        fecha_modificacion = NOW()
                    WHERE id_sesion = :id_sesion";
            $stmt = $conex->prepare($sql);
            $stmt->execute([
                ':id_microciclo'       => !empty($this->datos['id_microciclo']) ? (int)$this->datos['id_microciclo'] : null,
                ':id_grupo'            => (int)$this->datos['id_grupo'],
                ':fecha'               => $this->datos['fecha'],
                ':tipo_sesion'         => $this->datos['tipo_sesion'],
                ':calentamiento'       => $this->datos['calentamiento'] ?? null,
                ':vuelta_calma'        => $this->datos['vuelta_calma'] ?? null,
                ':volumen_planificado' => $volumen_planificado,
                ':observaciones'       => $this->datos['observaciones'] ?? null,
                ':id_entrenador'       => (int)$this->datos['id_entrenador'],
                ':duracion_minutos'    => $this->datos['duracion_minutos'] ?? null,
                ':id_sesion'           => $id_sesion,
            ]);

            $sqlDelete = "DELETE FROM series_sesion WHERE id_sesion = :id_sesion";
            $stmtDelete = $conex->prepare($sqlDelete);
            $stmtDelete->execute([':id_sesion' => $id_sesion]);

            $this->insertarSeries($conex, $id_sesion);

            $conex->commit();

            $this->notificarEventoSesion('editar_sesion', [
                'id_sesion'   => $id_sesion,
                'id_grupo'    => (int)$this->datos['id_grupo'],
                'fecha'       => $this->datos['fecha'],
                'tipo_sesion' => $this->datos['tipo_sesion'],
            ]);

            return true;
        } catch (PDOException $e) {
            if ($conex->inTransaction()) {
                $conex->rollBack();
            }
            error_log("Error en editarSesionP: " . $e->getMessage());
            return false;
        }
    }

    private function calcularVolumenes(array $series): array {
        $cal = 0;
        $prin = 0;
        $vuelta = 0;

        foreach ($series as $serie) {
            $vol = (int)($serie['repeticiones'] ?? 0) * (int)($serie['distancia_m'] ?? 0);
            if (($serie['bloque'] ?? '') === 'Calentamiento') {
                $cal += $vol;
            } elseif (($serie['bloque'] ?? '') === 'Principal') {
                $prin += $vol;
            } elseif (($serie['bloque'] ?? '') === 'VuletaCalma') {
                $vuelta += $vol;
            }
        }

        return [
            'calentamiento' => $cal,
            'principal'     => $prin,
            'vuelta_calma'  => $vuelta,
            'total'         => $cal + $prin + $vuelta,
        ];
    }

    private function insertarSeries(PDO $conex, int $id_sesion): void {
        $sql = "INSERT INTO series_sesion (
                    id_sesion, orden_ejecucion, bloque, id_drill,
                    ejercicio_descripcion, repeticiones, distancia_m,
                    descanso_seg, zona_intensidad, ritmo_objetivo
                ) VALUES (
                    :id_sesion, :orden, :bloque, :id_drill,
                    :descripcion, :repeticiones, :distancia,
                    :descanso, :zona, :ritmo
                )";
        $stmt = $conex->prepare($sql);

        $orden = 1;
        foreach ($this->series as $s) {
            $stmt->execute([
                ':id_sesion'   => $id_sesion,
                ':orden'       => $orden++,
                ':bloque'      => $s['bloque'] ?? '',
                ':id_drill'    => !empty($s['id_drill']) ? (int)$s['id_drill'] : null,
                ':descripcion' => empty($s['id_drill']) ? ($s['ejercicio_descripcion'] ?? null) : null,
                ':repeticiones'=> (int)($s['repeticiones'] ?? 0),
                ':distancia'   => (int)($s['distancia_m'] ?? 0),
                ':descanso'    => (int)($s['descanso_seg'] ?? 0),
                ':zona'        => $s['zona_intensidad'] ?? '',
                ':ritmo'       => $s['ritmo_objetivo'] ?? null,
            ]);
        }
    }

    private function notificarEventoSesion(string $evento, array $datos): void {
        try {
            if (!class_exists('\GrupoProyecto\SisBiomec\modelo\Notificacion')) {
                return;
            }

            switch ($evento) {
                case 'crear_sesion':
                    if (!empty($datos['id_sesion']) && !empty($datos['id_grupo'])) {
                        Notificacion::notificarSesionCreada(
                            $datos['id_sesion'],
                            $datos['id_grupo'],
                            $datos['fecha'],
                            $datos['tipo_sesion']
                        );
                    }
                    break;

                case 'iniciar_sesion':
                    if (!empty($datos['id_sesion']) && !empty($datos['id_grupo'])) {
                        Notificacion::notificarSesionIniciada(
                            $datos['id_sesion'],
                            $datos['id_grupo']
                        );
                    }
                    break;

                case 'completar_sesion':
                    if (!empty($datos['id_sesion']) && !empty($datos['id_grupo'])) {
                        Notificacion::notificarSesionCompletada(
                            $datos['id_sesion'],
                            $datos['id_grupo'],
                            $datos['volumen_ejecutado'] ?? 0
                        );
                    }
                    break;

                case 'cancelar_sesion':
                    if (!empty($datos['id_sesion']) && !empty($datos['id_grupo'])) {
                        Notificacion::notificarSesionCancelada(
                            $datos['id_sesion'],
                            $datos['id_grupo'],
                            $datos['fecha']
                        );
                    }
                    break;

                case 'editar_sesion':
                    if (!empty($datos['id_sesion']) && !empty($datos['id_grupo'])) {
                        Notificacion::notificarSesionEditada(
                            $datos['id_sesion'],
                            $datos['id_grupo'],
                            $datos['fecha'],
                            $datos['tipo_sesion']
                        );
                    }
                    break;
            }
        } catch (\Throwable $e) {
            error_log("Error en notificarEventoSesion: " . $e->getMessage());
        }
    }
}